...
function understandMessage(text) {
  ...
  if (isShortBackchannel(normalized) && recent) {
    return {
      ...
      detail: styleDetail({
        listener: "急がなくていいから、思った順で話してね。",
        chatty: "こういう間の返事も、会話の大事なところだよ。",
        coach: "今日は答えを出さなくても満点だよ。"
      }),
      question: pick([
        "今の話、続けるならどのへんからほどきたい？",
        "それって、まだ言葉にしたい感じがある？",
        "今は聞いててほしい？それとも少し一緒に整理する？"
      ])
    };
  }
...
function makeFollowUpQuestion(targetCharacter, tone) {
  ...
  const questions = {
    warm: [
      `${name}の今の気持ち、もう少し聞かせて〜。`,
      "その気持ちに名前をつけるなら、どんな名前になりそう？"
    ],
    cool: [
      "今日は決めなくてもいい。まず息しよ。",
      `${name}が今ほしいのは、聞いてもらうこと？それとも少し休むこと？`
    ],
    cheerful: [
      "よしよし、ここから一緒に軽くしていこう〜。",
      `${name}が少し楽になる方向でいこ。`
...
function extraListeningPrompts(targetCharacter) {
  const name = callName(targetCharacter);
  return [
    `${name}が満足するまで聞くよ〜。今はどの気持ちを先に見たい？`,
    "まだ言葉になってないところもあるかな？",
    `${name}のペースで大丈夫だよ。もう少しだけ続けてみる？`
  ];
}
...
characterForm.addEventListener("submit", (event) => {
  ...
  addMessage(
    "bot",
    `設定、反映したよ〜。${callName(character)}、この感じでゆるっと話してみるね。今の気持ち、まずどこから聞かせてもらおうか？`
  );
});
